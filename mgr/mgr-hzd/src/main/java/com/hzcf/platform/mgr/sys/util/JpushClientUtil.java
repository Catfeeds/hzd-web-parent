...
public class JpushClientUtil {
    private static final Log logger = Log.getLogger(JpushClientUtil.class);
    private final static String appKey = ConstantsDictionary.JPUSH_APPKEY;

    private final static String masterSecret = ConstantsDictionary.JPUSH_SECRET;
     ...
     * @return
     */
    public static int sendToAliasId(String aliasId, String notification_title, String msg_title, String msg_content, String extrasparam) {
        int result = 0;
        try{
            PushPayload pushPayload = JpushClientUtil.buildPushObject_all_alias_alert(aliasId, notification_title, msg_title, msg_content, extrasparam);
            PushResult pushResult = jPushClient.sendPush(pushPayload);
            if(pushResult.getResponseCode() == 200){
                result=1;
            }
        }catch (Exception e){
            logger.e("推送给别名为参数的用户异常:" ,e);
        }
        return result;
    }

    public static PushPayload buildPushObject_all_alias_alert(String aliasId, String notification_title, String msg_title, String msg_content, String extrasparam){
