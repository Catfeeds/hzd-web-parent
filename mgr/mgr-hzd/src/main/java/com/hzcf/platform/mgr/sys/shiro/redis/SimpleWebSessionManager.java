	...
	public void validateSessions() {
		...
		if (activeSessions != null && !activeSessions.isEmpty()) {
			for (Iterator<?> i$ = activeSessions.iterator(); i$.hasNext();) {
				...
				try {
					String sessionkey = CookieConstants.COOKIE_ID + session.getId();
					SessionKey key = new DefaultSessionKey(sessionkey);
					validate(session, key);
				} catch (InvalidSessionException e) {
					if (cacheManager != null) {
						SimpleSession s = (SimpleSession) session;
						String userkey = CookieConstants.USER_ID + session.getId();
						if (s.getAttribute(userkey) != null)
							cacheManager.getCache(null).remove(s.getAttribute(userkey));
					}
